...
if trim(request("butnvalue"))<>"" then 
		...
		userrank1="0"
      	...
      	if trim(lcase(request("butnvalue")))="accept1" then
			...
			insertval = 2
			sqlupdate="update TBL_SIS_Applications set User_Points="&trim(userrank)&",Date_Updated=getdate() ,Reason_Update='" & mystat1 & "',Updated_By='" & session("user_name") & "', user_status=2 where Rec_ID="&trim(request("ID"))&""
	...
	elseif trim(lcase(request("butnvalue")))="delete" then
			...
			userrank=0
			userrank1=0
			sqlupdate="update TBL_SIS_Applications set User_status="& insertval &" ,Date_Updated=getdate() , Reason_Update='" & mystat1 & "',Updated_By='" & session("user_name") & "'  where Rec_ID="&trim(request("ID"))&""
      	end if
		if trim(request("txtreason"))<>"" then
			mystat1 = trim(request("optionsgroup1"))&" "&trim(request("txtreason"))
		else
...
%>
<%
'Response.write " for tesiting Purpose"
'response.end
  ...
  <tr>
	...
	<%if trim(session("user_type")) ="4" or  trim(session("user_type")) ="5" or  trim(session("user_type")) ="7" then%>
	  ...
	  <tr>
            ...
            <td width="4%" align="center" height="1"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
            <td width="79%" height="1"><font face="verdana" size="2" color="#004080"><%=rs("Fathers_Name")%></td>
          </tr>
          <tr>
...
<tr align="center">
	   ...
	   <tr>
		...
		<INPUT Type="radio" id="optionsgroup1" name="optionsgroup1" value="N-20" >N-20&nbsp;&nbsp;
