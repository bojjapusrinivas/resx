...
function submitval(actval)
{
	...
	if (updateval==true)
	{
		...
		window.frmsis.action="Frm_SIS_ViewATN.asp";
...
function OpenWin(num)
	{ 
		myWin = open("AnAApplicantIntinfo.asp?ID="+num+"#callID1" ,"userwin","width=780,top=5,left=5,height=550,status=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
	}
</script>
...
<table border="1" width="95%" bordercolor="#005563" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#005563"><p align="center"><strong><big><font color="#FFFFFF">Test Evaluation <!--[<%'=ttype%>]--></font></big></strong></td>
  </tr>
  <tr>
	...
	<tr>
		...
		if trim(session("user_type"))="1" then
			if trim(request("candname"))<>"" then		
				usql1="select * from TBL_SIS_Applications  where (testwriteupdate>='5/21/2018' and user_status =11 and user_name like ('%"&trim(Request("candname"))&"%')) and (applicationfor='Hyderabad' or applicationfor='BizAcumen' or applicationfor='Campus') order by rec_id"
			else
				usql1="select *,dbo.ReturnAppearedHydDetails(Rec_id) as appeared from TBL_SIS_Applications  where User_Status=11 and testwriteupdate>='5/21/2018' and  ANA_No <> '' and (applicationfor='Hyderabad' or applicationfor='BizAcumen' or applicationfor='Campus')  order by rec_id "
			end if
		else
			if trim(request("candname"))<>"" then
				usql1="select * from TBL_SIS_Applications  where (testwriteupdate>='5/21/2018' and user_status =11 and user_name like ('%"&trim(Request("candname"))&"%'))and (applicationfor='Hyderabad' or applicationfor='BizAcumen' or applicationfor='Campus')  order by rec_id"
			else
				usql1="select *,dbo.ReturnAppearedHydDetails(Rec_id) as appeared from TBL_SIS_Applications  where  ( testwriteupdate>='5/21/2018' and ANA_No <> '') and (applicationfor='Hyderabad' or applicationfor='BizAcumen' or applicationfor='Campus')  order by rec_id "
					'user_status =2 and user_status =11 and
			end if
		end if
		''response.write usql1
		rsDisp.Open usql1
		''paging
		'rsDisp.movefirst<script type="text/javascript">
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_SIS_ViewATN.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&candname=<%=trim(request("candname"))%>&ttype=<%=ttype1%>"><%=i%></a>			
		...
		Response.Write ("<tr>")
			...
			Response.Write ("<th><font face=""verdana"" size=""2"">ANA ID</font></th>")
			if trim(session("user_type")) ="4" then
			Response.Write ("<th><font face=""verdana"" size=""2"">Name/IGM's Rank</font></th>")
			else
			Response.Write ("<th><font face=""verdana"" size=""2"">Education UG</font></th>")
			end if
			Response.Write ("<th><font face=""verdana"" size=""2"">Post Applied</font></th>")
			Response.Write ("<th><font face=""verdana"" size=""2"">Resume & Photo</font></th>") 
			'Response.Write ("<th><font face=""verdana"" size=""2"">Approved by</font></th>") 
		...
		IF not rsDisp.EOF THEN
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Response.Write ("<tr bgcolor='white'>")
					...
					Response.Write ("<td align='center'><font face='verdana' size='2'>"& trim(rsDisp("ANA_No")) &"</font></td>")
					if trim(session("user_type")) ="4" then
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("user_Name") & " [" & rsDisp("User_Points") & "]" &"</font></td>")
					else
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("Education_ug") &"</font></td>")
					end if
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("Post_Applied") &"</font></td>")
					if trim(rsDisp("appeared")) ="Both" then
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("appeared") &"</font></td>")
					Response.Write ("<td align='center'><font face='verdana' size='2'>"& UsrStat  &"</font></td>")
					elseif trim(rsDisp("appeared")) ="Only Photos" then
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("appeared") &"</font></td>")
					Response.Write ("<td align='center'><font face='verdana' size='2'>"& UsrStat  &"</font></td>")
					else
					Response.Write ("<td align='left'><font face='verdana' size='2' color='Red'><b>"& rsDisp("appeared") &"</b></font></td>")
					Response.Write ("<td align='center'><font face='verdana' size='2' color='Red'>Invalid</font></td>")
					end if
					'Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("Updated_BY") &"</font></td>")
					
					if trim(rsDisp("User_Status"))="4" then
						Response.Write ("<td align='center'><a href='javascript:OpenWin("& rsDisp("Rec_id") &");'><font face='verdana' size='2'>View</font></a> | <a href='AnAApplicantinfo.asp?stat=A&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Accept</font></a> | <a href='FRM_SIS_Evaluation.asp?stat=D&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Decline</font></a> | <a href='FRM_SIS_Evaluation.asp?stat=H&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Hold</font></a></td>")
					else
					     if trim(rsDisp("appeared")) ="Both" then
						...
						Response.Write ("<td align='center'><a href='javascript:OpenWin("& rsDisp("Rec_id") &");'><font face='verdana' size='2'>View</font></a></td>")
					        ...
					        Response.Write ("<td align='center'><font face='verdana' size='2'>View</font></td>")
					     end if
					end if
				Response.Write ("</tr>")
				j=j+1
