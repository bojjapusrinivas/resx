...
<td colspan="7">  <div align="center"><center><p><font color="#004080"><strong>Candidate Name/ID :</strong></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<tr>
		...
		if trim(session("user_type"))="1" then
			if trim(request("candname"))<>"" then		
				usql1="select *,dbo.ReturnSlotDetails(b.SelectedSlot) as SlotDetails,dbo.ReturnCountofhydSlots(b.SelectedSlot) as slotCount from TBL_SIS_Applications a, Interview_Hyd_SelSlots b where b.Rec_ID=a.Rec_ID and (b.Date_Updated<>'' or b.Date_Updated is not null) and b.SelectedSlot<>'' " & whrcnd & " order by b.SelectedSlot"
			else
			...
			if trim(request("candname"))<>"" then
				usql1="select *,dbo.ReturnSlotDetails(b.SelectedSlot) as SlotDetails,dbo.ReturnCountofHydSlots(b.SelectedSlot) as slotCount from TBL_SIS_Applications a, Interview_Hyd_SelSlots b where b.Rec_ID=a.Rec_ID and (b.Date_Updated<>'' or b.Date_Updated is not null) and b.SelectedSlot<>'' " & whrcnd & " and user_status=11 order by b.SelectedSlot,a.rec_id"
			else
				usql1="select *,dbo.ReturnSlotDetails(b.SelectedSlot) as SlotDetails,dbo.ReturnCountofHydSlots(b.SelectedSlot) as slotCount from TBL_SIS_Applications a, Interview_Hyd_SelSlots b where b.Rec_ID=a.Rec_ID and (b.Date_Updated<>'' or b.Date_Updated is not null) and b.SelectedSlot<>'' and User_Status=11 " & whrcnd & " order by b.SelectedSlot,a.rec_id"
			end if
		end if
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_SIS_ViewSelSlotsInt.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&id=<%=Request("ID")%>&candname=<%=trim(request("candname"))%>&ttype=<%=ttype1%>"><%=i%></a>			
		...
		IF not rsDisp.EOF THEN
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Response.Write ("<tr bgcolor='white'>")
					...
					'if trim(ucase(rsDisp("mail_sent")))="Y" then
					'	Response.Write ("<td align='center'>Add</td>")
					'else
						Response.Write ("<td align='center'><a href=javascript:OpenWinInt("& rsDisp("Rec_id") & ");><font face='verdana' size='2'>Add</font></a></td>")
					'end if
				Response.Write ("</tr>")
				j=j+1
				no=no+1
