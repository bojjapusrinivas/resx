...
if trim(request("butnvalue"))<>"" then
	...
	'response.end
		...
		''MessageBody1 = MessageBody1 & "<tr><td width='100%'>&nbsp;</td><td>&nbsp;</td></tr>"
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>Dear "&request("txtName")&",</font></b><br><br></td></tr>"
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>Thank you for taking out time to apply online for the position of External Writers. We have carefully considered your application and regret to inform you that your candidature does not form a fit with our present requirements. <br><br></td></tr>"
		...
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>Regards,</td></tr>"
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>Coordinator, eNet Services</td></tr>"
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>SIS Infotech Private Limited</td></tr>"
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'><a href='mailto:enet.svcs@sisinfotech.com'>enet.svcs@sisinfotech.com</a></td></tr>"
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'><a href='http://www.sisinfotech.com'>www.sisinfotech.com</a></td></tr>"
		MessageBody1 = MessageBody1 & "</table>"
	    call sendemail1("careers@sisinfotech.com",trim(request("txtemail")),"SIS Infotech Pvt Ltd. – Online Application Status",MessageBody1)	
		''call sendemail1("careers@sisinfotech.com","chv@sisinfotech.com","SIS Infotech Pvt Ltd. – Online Application Status",MessageBody1)	
	...
	elseif trim(lcase(request("butnvalue")))="hold" then
		...
		mystat1 = "hold"
		insertval = 6
	elseif trim(lcase(request("butnvalue")))="approved fully" then
		Mystat = "Accept2"
	...
	if trim(request("txtreason"))<>"" then
		mystat1 = trim(request("txtreason")) & mystat1 & " approved by " & session("user_name")
	Else
		mystat1 = mystat1 & " approved by " & session("user_name")
	end if
		dim recID
		...
		Response.write ("update TBL_SIS_FreeLancerNew set Points_Scored1=" & trim(request("UserRank")) &", user_status=" & insertval & ", Reason_Update='"  & mystat1 & "', Date_Updated=getdate() where Rec_id =" & trim(recID) & "")
		'response.end
		con.execute("update TBL_SIS_FreeLancerNew set Points_Scored1=" & trim(request("UserRank")) &", user_status=" & insertval & ", Reason_Update='"  & mystat1 & "', Date_Updated=getdate() where Rec_id =" & trim(recID) & "")
...
''
		...
		dim x
		x = cint(Request("ID"))+1
		response.redirect ("Applicantinfo_FinalRanks.asp?ID="&trim(x)&"&SUB=1#callID1")
end if
%>
<html>
<head>
...
function submitval(actval,recID)
{
	...
	if (updateval==true)
	{
		...
		window.frmsis.action="Applicantinfo_FinalRanks.asp?ID="+recID+"&butnvalue="+actval;
...
if trim(Request("ID"))<>"" then
	SQL = "SELECT *,isnull(points_scored1,0) as chkPoints FROM TBL_SIS_FreeLancernew where user_status =2 and Rec_ID="&trim(Request("ID"))
else
	SQL = "SELECT *,isnull(points_scored1,0) as chkPoints FROM TBL_SIS_FreeLancernew where user_status =2 and  user_name like ('%"&trim(Request("candname"))&"%')"
end If
'response.write sql
rs.OPEN SQL,con
If Not RS.EOF Then
...
<table border="1" width="90%" cellspacing="0" cellpadding="0" bordercolor="#000080" class=mystyle>
  <tr>
	...
	<tr align="center">
		...
		<!--table width="100%"><tr>
		<td align="left"><a href="Applicantinfo_FinalRanks.asp?ID=<%=cint(trim(request("ID")))-1%>#callID1"><b> &laquo; Previous  </b></a></td>
		<td align="right"><a href="Applicantinfo_FinalRanks.asp?ID=<%=cint(trim(request("ID")))+1%>#callID1"><b> Next &raquo; </b></a></td>
		</tr></table-->
		</td>
        </tr>
		  <tr>
	      ...
	      <%if (trim(session("utype"))="2" or trim(session("utype"))="3"  or trim(session("utype"))="4" or trim(session("utype"))="5") then%>
          <tr>
            <td width="100%"><font face="verdana" size="2" color="#804040"><%=UsrStat%>&nbsp;Stage I :&nbsp; <%=rs("Updated_BY")%></font></td>
          </tr>
           
          <tr>
            <td width="100%"><font face="verdana" size="2" color="#804040"><%=UsrStat%>&nbsp;Stage II:&nbsp; <%=rs("Reason_Update")%></font></td>
          </tr>
		  <tr>
	...
	<font color="#FF0000"><small>*</small></font><br><font color=red>You are requested NOT to paste your resume in the following box. Pasting your resume here will automatically disqualify you from our recruitment process.</font>
      ...
      <tr align="center">
        <td width="100%" height="0" border=""><form method="post" action="Applicantinfo_FinalRanks.asp" name="frmsis">
<tr align="center">
...
dim MYValID,MYValID1
	  ...
	  <tr>
        <td width="100%"><big><font face="verdana" size="2" color="blue"><strong>Application :&nbsp; <%=rs("Rec_ID")%> : <%'=rs("user_Name")%> : <%'=edu%> <%'=edu1%></strong></font></big></td>
      </tr>
	
      <tr align="center">
        <td width="100%" height="0" align="right">
		<!--table width="100%"><tr>
		<td align="left"><a href="Applicantinfo_FinalRanks.asp?ID=<%=trim(MYValID1)%>#callID1"><b> &laquo; Previous  </b></a></td>
		<td align="right"><a href="Applicantinfo_FinalRanks.asp?ID=<%=trim(MYValID)%>#callID1"><b> Next &raquo; </b></a></td>
		</tr></table-->
		</td>
      </tr>
    </table>
...
Else %>
	...
	<br><p align="left">
		...
		'''''''''''''''''''''''''''''
		'''' for next forward value
		'''''''''''''''''''''''''''''
		set RSNavigate = server.createobject("ADODB.Recordset")
		RSNavigate.Activeconnection = con
		''if cint(trim(session("utype")))=4 then
		''	SqlNavigate1 = "SELECT top 1 rec_id FROM TBL_SIS_FreeLancernew where user_status=2 and updated_by='" & session("user_name") & "' and Rec_ID>="&  MYVID1 & " order by rec_id"
		''else
			SqlNavigate1 = "SELECT top 1 rec_id FROM TBL_SIS_FreeLancernew where user_status=2 and updated_by='" & session("user_name") & "' and Rec_ID>="&  MYVID1 & " order by rec_id"
		''end if 
		RSNavigate.open sqlnavigate1
		''response.write SqlNavigate1
		if not RSNavigate.eof then
			MYVID1 = trim(RSNavigate("Rec_id"))
		end if 
		set RSNavigate = nothing
		

		if MYVID1=1 then%>
			<!--a href="Applicantinfo_FinalRanks.asp?ID=<%=trim(MYVID1)%>#callID1"><b> Next &raquo;</b></a-->
		<%else%>
        	<!--a href="Applicantinfo_FinalRanks.asp?ID=<%=trim(MYVID)%>#callID1"><b> &laquo; Previous </b></a-->
		<%end if%>
      </p>
<% End If%>
</body>
