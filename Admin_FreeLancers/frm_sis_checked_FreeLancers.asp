...
if (trim(request("Flno"))<>"") then
	...
	con.close()
	response.redirect("frm_sis_checked_FreeLancers.asp")
	%>
...
function OpenSend(num)
	...
	{ 
		...
		window.frmsis.method="post";
		window.frmsis.action="frm_sis_checked_FreeLancers.asp?Flno=" + num + "&flName=" + document.frmsis['txt' + num].value;
		window.frmsis.submit();
		return true;
 ...
 <tr>
  ...
  <tr>
	...
	<tr>
		...
		usql1="select *,isnull(points_scored1,0) as chkPoints from TBL_SIS_FreeLancernew where  Rec_id >0 and updated_by like '%" & session("user_name") &"%' ORDER BY Date_Updated, User_Status, Rec_id" 'rec_id"   'Points_Scored1 desc,
		''usql1="select *,isnull(points_scored1,0) as chkPoints from TBL_SIS_FreeLancernew where  user_status="&TStat&" and Rec_id >2895  order by points_scored1, Rec_id" 'rec_id"   'Points_Scored1 desc,
		end if
		rsDisp.Open usql1
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="frm_sis_checked_FreeLancers.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&ttype=<%=trim(request("ttype"))%>&vtype=<%=trim(request("vtype"))%>"><%=i%></a>			
		...
		Response.Write ("<tr>")
			...
			Response.Write ("<th><font face=""verdana"" size=""2"">User Status</font></th>")
		...
		IF not rsDisp.EOF THEN					
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Response.Write ("<tr bgcolor='white'>")					
					...
					Response.Write ("<td align='left'><font face='verdana' size='2'>&nbsp;&nbsp;"& UsrStat &"</font></td>")
