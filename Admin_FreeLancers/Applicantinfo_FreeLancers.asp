...
if trim(request("butnvalue"))<>"" then
	...
	'response.end
		...
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>Coordinator, eNet Services</td></tr>"
		MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>BizAcumen Research Pvt. Ltd.</td></tr>"
		'MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'><a href='mailto:enet.svcs@BizAcumen.com'>enet.svcs@BizAcumen.com</a></td></tr>"
		'MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'><a href='http://www.BizAcumen.com'>www.BizAcumen.com</a></td></tr>"
		MessageBody1 = MessageBody1 & "</table>"
	      ''''  call SendGodaddyMail("careers@BizAcumen.com",trim(request("txtemail")),"BizAcumen Research Pvt Ltd. – Online Application Status",MessageBody1)	

			'''call sendemail1("careers@BizAcumen.com","chv@sisinfotech.com","BizAcumen Research Pvt Ltd. – Online Application Status",MessageBody1)	
...
'''	response.write "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)  
		...
		cmdEdit.ActiveConnection = con	
			with cmdEdit
				...
				.Parameters.Append .CreateParameter ("@MySpecial_Skills",advarchar,adparamInputOutput,8000,"Skills")
		...
		''response.write("update TBL_SIS_FreeLancerNew set Points_Scored1=" & UserRank &",Updated_By='" & trim(session("user_name")) & "',Reason_Update='Accept Stage 2 Approved by " & session("user_name") & "',Date_Updated=getdate(), user_status=" & insertval & " where user_status=1 and Rec_id =" & trim(recID) & "")
		''		response.end
			con.execute("update TBL_SIS_FreeLancerNew set Points_Scored1=" & UserRank &",Updated_By='" & trim(session("user_name")) & "',Reason_Update='Accept Stage 2 Approved by " & session("user_name") & "',Date_Updated=getdate(), user_status=" & insertval & " where user_status=1 and Rec_id =" & trim(recID) & "")
...
if trim(Request("ID"))<>"" then
	''SQL = "SELECT * FROM TBL_SIS_FreeLancernew where user_status not in (0,7,8,9) and Rec_ID="&trim(Request("ID"))
	SQL = "SELECT * FROM TBL_SIS_FreeLancernew where user_status  in (1) and Rec_ID="&trim(Request("ID"))
else
	SQL = "SELECT * FROM TBL_SIS_FreeLancernew where user_status not in (0,7,8,9) and  user_name like ('%"&trim(Request("candname"))&"%')"
end if
rs.OPEN SQL,con
If Not RS.EOF Then
Dim ID
...
<tr align="center">
	  ...
	  <tr>
            <td width="100%"><font face="verdana" size="2" color="#004080">Rank II :</font> <select name="UserRank">
