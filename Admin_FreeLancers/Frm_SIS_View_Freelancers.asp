  ...
  <tr>
	...
	<tr>
		...
		if trim(session("user_type"))="1" then
			if trim(request("candname"))<>"" then		
				usql1="select * from TBL_SIS_FreeLancernew where user_status not in (0,7,8,9) and user_name like ('%"&trim(Request("candname"))&"%') order by rec_id"
			elseif trim(request("Cityname"))<>"" then
				usql1="select * from TBL_SIS_FreeLancernew where user_status not in (0,7,8,9) and User_City like ('%"&trim(Request("Cityname"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_FreeLancernew where user_status not in (0,7,8,9) and User_Status=1 order by rec_id"
			end If
		elseif trim(session("user_type"))="4" then
			if trim(request("candname"))<>"" then		
		...
		Response.Write ("<tr>")
			...
			Response.Write ("<th><font face=""verdana"" size=""2"">Advertisement</font></th>")
			Response.Write ("<th><font face=""verdana"" size=""2"">Options</font></th>")
		Response.Write ("</tr>")
		IF not rsDisp.EOF THEN					
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Dim allapps
				allapps=""
				' Commented for Interface is became slow
			...
			'	Else
			'	rsAllAps.Open "select * from vw_OnlineNaukrieNet_apps where (user_phone ='" & trim(rsDisp("user_phone")) & "' or user_email ='" & trim(rsDisp("User_Email")) & "' or user_alternateemail ='" & rsDisp("user_alternateemail") & "')  and user_status <> 8"
			'	End if
			...
			''  Commented end	
				Response.Write ("<tr bgcolor='white'>")
					...
					Response.Write ("<td align='left'>&nbsp;<font face='verdana' size='2'>"& rsDisp("See_Advertisement") &"</font></td>")
					if trim(rsDisp("User_Status"))="1" then
