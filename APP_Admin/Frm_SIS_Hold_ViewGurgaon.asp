...
if trim(request("stat")) = "A" then
 '' response.write trim(request("stat"))
	...
	con.execute("update TBL_SIS_Analysts_Gurgaon set user_status=1 where Rec_id =" & trim(request("recID")))
	con.close()
	response.redirect("FRM_SIS_Hold_ViewGurgaon.asp?ttype=9")	
end if
	...
	if trim(var)<>"butnvalue" then
		...
		'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
			...
			cmdEdit.ActiveConnection = con			
				with cmdEdit 				
					.CommandText = "SP_TBL_SIS_Analysts_Gurgaon"
			...
			rsDisp.CursorType = 3
			usql1="select * from TBL_SIS_Analysts_gurgaon where rec_id = "&trim(recID)&""
			rsDisp.open usql1
			...
			''if loadedval=1 then
				...
				End with
				set mymail = nothing 
				con.execute("update TBL_SIS_Analysts_gurgaon set User_Status=8,Date_Updated=getdate(), Reason_Update=Reason_Update + ' Mail Sent ' where Rec_ID=" & trim(recID))
...
<table border="1" width="95%" bordercolor="#005563" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#005563"><p align="center"><strong><big><font color="#FFFFFF">Gurgaon Block Requests</font></big></strong></td>
  </tr>
  ...
  <tr>
	...
	<tr>
		...
		''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
		''''' UPDATED on 25 November 2009 By OBILI'''''''''''''''''''''''''''''''''''''''''''''''''
		'''' Changed from TBL_SIS_Analysts_gurgaon to TBL_SIS_Applications where applicationfor='Gurgaon'
		'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
		''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''	
		if trim(TStat)="4"  then		
			usql1="select * from TBL_SIS_Applications where applicationfor='Gurgaon' and rec_id <=70 order by rec_id"
		elseif trim(TStat)="0"  then
			usql1="select * from TBL_SIS_Applications where applicationfor='Gurgaon' and rec_id>869 order by Date_Created desc"
		elseif trim(TStat)="6"  then
			usql1="select * from TBL_SIS_Applications where applicationfor='Gurgaon' and ANA_No <> '' and rec_id >70 order by Rec_id"
		elseif trim(TStat)="2"  then
				usql1="select * from TBL_SIS_Applications where applicationfor='Gurgaon' and user_status="&TStat&" and rec_id>869  order by rec_id"
		else
			usql1="select * from TBL_SIS_Applications where applicationfor='Gurgaon' and user_status="&TStat&" and rec_id>869  order by rec_id"
			''usql1="select * from TBL_SIS_Analysts_gurgaon where user_status="&TStat&" and rec_id>869  order by rec_id"
		end if
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_SIS_Hold_ViewGurgaon.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&ttype=<%=trim(request("ttype"))%>"><%=i%></a>			
					...
					if trim(TStat)="0" then
					  ...
					  if trim(TStat)="6" then
						...
						if trim(rsDisp("Admin_Status"))="1" then
							...
							elseif trim(rsDisp("User_Status"))="9" then
								Response.Write ("<td align='center'><a href='javascript:OpenWin("& rsDisp("Rec_id") &");'><font face='verdana' size='2'>View</font></a>| <a href='FRM_SIS_Hold_ViewGurgaon.asp?stat=A&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Release</font></a> </td>") '| <a href='FRM_SIS_Final_View.asp?actstat="&rsDisp("User_status")&"&stat=A&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Email</font></a></td>")
							end if
						else
