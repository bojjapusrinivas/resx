  ...
  <tr>
	...
	<tr>
		...
		if trim(TStat)="4"  then		
			usql1="select * from TBL_SIS_Analysts where admin_status=2 and rec_id>5728 order by rec_id"
		elseif trim(TStat)="0"  then
			usql1="select * from TBL_SIS_Analysts where rec_id>5728 order by Date_Created desc"
		elseif trim(TStat)="6"  then
			usql1="select * from TBL_SIS_Analysts where ANA_No <> '' order by Date_Created desc"
		else
			usql1="select * from TBL_SIS_Analysts where user_status="&TStat&" and rec_id>5728 order by rec_id"
		end if
		rsDisp.Open usql1
		''paging
		'rsDisp.movefirst
