...
function submitval(actval)
{
	...
	if (updateval==true)
	{
		...
		window.frmsis.action="Frm_SIS_ViewATN_DONE.asp";
	...
	<tr>
		...
		if trim(session("user_type"))="4" or trim(session("user_type"))="0" then
			if trim(request("candname"))<>"" then		
				''usql1="select * from TBL_SIS_Analysts where (testwriteupdate>='1/21/2009' and user_status >=2 and user_name like ('%"&trim(Request("candname"))&"%')) order by testwriteupdate"
				usql1="select * from TBL_SIS_Applications where (user_status >=2 and User_Points is not null and user_name like ('%"&trim(Request("candname"))&"%')) order by testwriteupdate desc"
			else
				'usql1="select * from TBL_SIS_Analysts where User_Status=1 and testwriteupdate>='1/21/2009' and user_status =2 and ANA_No <> ''  order by testwriteupdate "
				usql1="select * from TBL_SIS_Applications where User_Points is not null and ANA_No <> ''  order by testwriteupdate desc"
			end if
		else
			if trim(request("candname"))<>"" then
				'usql1="select * from TBL_SIS_Analysts where (testwriteupdate>='1/21/2009' and user_status >=2 and user_name like ('%"&trim(Request("candname"))&"%')) and user_status=1 order by testwriteupdate"
				usql1="select * from TBL_SIS_Applications where (user_status >=2 and User_Points is not null and user_name like ('%"&trim(Request("candname"))&"%')) and user_status=1 order by testwriteupdate desc"
			else
				''usql1="select * from TBL_SIS_Analysts where  ( testwriteupdate>='1/21/2009' and ANA_No <> '')  order by testwriteupdate "
				usql1="select * from TBL_SIS_Applications where  (ANA_No <> '' and User_Points is not null)  order by testwriteupdate desc"
					'user_status =2 and
			end if
		end if
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_SIS_ViewATN_DONE.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&candname=<%=trim(request("candname"))%>&ttype=<%=ttype1%>"><%=i%></a>			
		...
		Response.Write ("<tr>")
			...
			Response.Write ("<th><font face=""verdana"" size=""2"">Test Date</font></th>")
			end if
			Response.Write ("<th><font face=""verdana"" size=""2"">Post Applied</font></th>")
			Response.Write ("<th><font face=""verdana"" size=""2"">Education PG</font></th>") 
		...
		IF not rsDisp.EOF THEN
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Response.Write ("<tr bgcolor='white'>")
					...
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("testwriteupdate") &"</font></td>")
					end if
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("Post_Applied") &"</font></td>")
					Response.Write ("<td align='left'><font face='verdana' size='2'>"& rsDisp("Education_PG") &"</font></td>")
