...
if trim(request("butnvalue"))<>"" then
	...
	elseif trim(lcase(request("butnvalue")))="block" then
        ...
        elseif trim(lcase(request("butnvalue")))="accept" then
			...
			userrank=request("UserRank11")& ""
			insertval = 2
			sqlupdate="update TBL_SIS_Applications set Points="&trim(userrank)&" where Rec_ID="&trim(request("ID"))&""
        elseif trim(lcase(request("butnvalue")))="accept22" then
			Mystat = "Approve"
			'mystat1 = "Approved"
			mystat1=request("txtreason")
			userrank=request("UserRank11")
			insertval = 2
			sqlupdate="update TBL_SIS_Applications set Points_US="&trim(userrank)&" where Rec_ID="&trim(request("ID"))&""
	...
	elseif trim(lcase(request("butnvalue")))="hold" then
		...
		cmdEdit.ActiveConnection = con	
			with cmdEdit
				.CommandText = "SP_TBL_SIS_Analysts_GurgaonRanks"
				...
				.Parameters.Append .CreateParameter ("@My_Scored",advarchar,adparamInputOutput,4,userrank)
				.Parameters.Append .CreateParameter ("@My_Scored1",advarchar,adparamInputOutput,4,userrank1)	
		...
		dim x
		x = cint(Request("ID"))+1
		response.redirect ("ApplicantInfoGurgaon.asp?ID="&trim(x)&"&SUB=1#callID1")
end if
%>
<html>
<head>
...
function submitval(actval,recID)
{
...

	{
		...
		window.frmsis.action="applicantinfoGurgaon.asp?ID="+recID+"&butnvalue="+actval;
...
if trim(Request("ID"))<>"" then
	'SQL = "SELECT * FROM TBL_SIS_Applications where applicationfor='gurgaon' and Rec_ID="&trim(Request("ID"))
	SQL = "SELECT * FROM TBL_SIS_Applications where (applicationfor='gurgaon' or applicationfor ='Bizacumen') and location ='gurgaon' and Rec_ID="&trim(Request("ID"))
else
	'SQL = "SELECT * FROM TBL_SIS_Applications where  user_name like ('%"&trim(Request("candname"))&"%')"
	SQL = "SELECT * FROM TBL_SIS_Applications where (applicationfor='gurgaon' or applicationfor ='Bizacumen') and location ='gurgaon' and user_name like ('%"&trim(Request("candname"))&"%')"
end if
'response.write sql
rs.OPEN SQL,con
'Response.write(" for testing purpose")
If Not RS.EOF Then
...
if trim(rs("User_Status"))="1" then
	UsrStat = "Pending"
elseif trim(rs("User_Status"))="0" then
...
elseif trim(rs("User_Status"))="5" then
	UsrStat = "Hold"
elseif trim(rs("User_Status"))="8" then
	UsrStat = "Mail Sent"
end if
'response.write UsrStat & " for test"
%>
...
<table border="1" width="90%" cellspacing="0" cellpadding="0" bordercolor="#000080" class=mystyle>
  <tr>
    ...
    cellspacing="0" cellpadding="5">
      ...
      <tr align="center">
        ...
        <table border="0" width="95%" cellspacing="0" cellpadding="0">
          ...
          <%if (trim(rs("User_Status"))="2" or trim(rs("User_Status"))="3" or trim(rs("User_Status"))="5" or trim(rs("User_Status"))="8") then%>
          <tr>
	...
	<% 
	if (trim(session("user_type")) ="4" or trim(session("user_type")) ="2" or trim(session("user_type")) ="0") then%>
	  ...
	  <tr>
		  ...
		  <tr>
            ...
            <td width="4%" align="center" height="1"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
            <td width="79%" height="1"><font face="verdana" size="2" color="#008080"><%=Replace(rs("fathers_occupation"),"##"," & ")%></td>
          </tr>
          <tr>
...
<tr align="center">
	 ...
	 <tr>
	    <td>&nbsp;<font face="verdana" size="2" color="#004080"><b>Ranks:</b></font></td>
          </tr>

	 <tr>
            <td width="100%"><font face="verdana" size="2" color="#004080">IGM Rank :</font> <%if (isnull(trim(rs("User_Points")))=true) and ((trim(rs("User_Status"))="1") or (trim(rs("User_Status"))="5") or (trim(rs("User_Status"))="3") or (trim(rs("User_Status"))="8"))  then%> <select name="UserRank11">
		<option value="0" <%=selected(trim(rs("user_Points")),"0")%>></option>
...
Response.write trim(rs("user_Points"))
	...
	<%''if (trim(session("user_type")) ="4" and isnull(trim(rs("Points")))=false ) then%>
	<%if (trim(session("user_type")) ="4" ) then%>
		<font face="verdana" size="2" color="#004080">RR Rank :</font> <select name="UserRank11">
		<option value="" <%'=selected(trim(rs("Points")),"")%>></option>
	...
	<%else
		Response.write "RR Rank: "& trim(rs("user_Points_US"))

	end if%>
</td>
    </tr>
<tr>      
<%if ( (trim(rs("User_Status"))="1") or (trim(rs("User_Status"))="2") or (trim(rs("User_Status"))="5") or (trim(rs("User_Status"))="3")or (trim(rs("User_Status"))="8") ) then
                 if trim(session("user_type")) ="4" then
			Response.Write ("<td colspan=7 align=left><input type=""button"" name=""butnvalue"" value=""IGMs Accept"" onclick=""javascript:submitval('Accept','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""RR Accept"" onclick=""javascript:submitval('Accept22','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Reject"" onclick=""javascript:submitval('Decline','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Hold"" onclick=""javascript:submitval('Hold','"&rs("Rec_ID")&"');""></td>")	
		else
			Response.Write ("<td colspan=7 align=left><input type=""button"" name=""butnvalue"" value=""IGMs Accept"" onclick=""javascript:submitval('Accept','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Reject"" onclick=""javascript:submitval('Decline','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Hold"" onclick=""javascript:submitval('Hold','"&rs("Rec_ID")&"');""></td>")	
		end if
end if	%>
</tr>
...
<%if (trim(rs("User_Status"))="2" or trim(rs("User_Status"))="3" or trim(rs("User_Status"))="4" or trim(rs("User_Status"))="6" or trim(rs("User_Status"))="5") then%>
	...
	<%end if%>
      ...
      <tr align="center">
		...
		<a href="ApplicantInfoGurgaon.asp?ID=<%=trim(MYValID1)%>#callID1"><b> &laquo; Previous  </b></a></td>
		<%end if%>
		<td align="right"><a href="ApplicantInfoGurgaon.asp?ID=<%=trim(MYValID)%>#callID1"><b> Next &raquo; </b></a></td>
		</tr></table>
		</td>
      </tr>
...
Else %>
	...
	<br><p align="left">
		...
		if MYVID1 > 474 then%>
			<a href="ApplicantInfoGurgaon.asp?ID=<%=trim(MYVID1)%>#callID1"><b> Next &raquo;</b></a>
		<%elseif MYVID > 474 then %>
        	<a href="ApplicantInfoGurgaon.asp?ID=<%=trim(MYVID)%>#callID1"><b> &laquo; Previous </b></a>
		<%end if%>
      </p>
<% End If%>
</body>
