	...
	<tr>
		...
		if trim(session("user_type"))="1" then
			if trim(request("candname"))<>"" then		
				...
				usql1="select * from TBL_SIS_Analysts where (user_status >=2 and user_name like ('%"&trim(Request("candname"))&"%')) order by testwriteupdate desc"
			else
				...
				usql1="select * from TBL_SIS_Analysts where User_Status=1  and user_status =2 and ANA_No <> ''  order by testwriteupdate desc"
			end if
		else
			if trim(request("candname"))<>"" then
				...
				usql1="select * from TBL_SIS_Analysts where (user_status >=2 and user_name like ('%"&trim(Request("candname"))&"%')) and user_status=1 order by testwriteupdate desc"
			else
				''usql1="select * from TBL_SIS_Analysts where  ( testwriteupdate>='1/21/2009' and ANA_No <> '')  order by testwriteupdate "
				usql1="select * from TBL_SIS_Analysts where  (  ANA_No <> '')  order by testwriteupdate desc"
					'user_status =2 and
			end if
		end if
