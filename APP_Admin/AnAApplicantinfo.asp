...
if trim(request("butnvalue"))<>"" then
		...
		userrank1="0"
      	if trim(lcase(request("butnvalue")))="accept1" then
			...
			insertval = 2
			sqlupdate="update TBL_SIS_Analysts set User_Points="&trim(userrank)&",Updated_By='" & trim(session("User_Name")) & "' where Rec_ID="&trim(request("ID"))&""
...
'response.end
	...
	'	response.end
		...
		dim x
		x = cint(Request("ID"))+1
		response.redirect ("AnAApplicantInfo.asp?ID="&trim(Request("ID"))&"&SUB=1#callID1")
end if
%>
<html>
<head>
...
function submitval(actval,recID)
{
	...
	//{
		...
		window.frmsis.action="anaapplicantinfo.asp?ID="+recID+"&butnvalue="+actval;
...
if trim(Request("ID"))<>"" then
	'SQL = "SELECT * FROM TBL_SIS_Analysts where testwriteupdate>='6/10/2007' and Rec_ID="&trim(Request("ID")) & ""
	SQL = "SELECT * FROM TBL_SIS_Analysts where  Rec_ID="&trim(Request("ID")) & ""
else
	'SQL = "SELECT * FROM TBL_SIS_Analysts where testwriteupdate>='6/10/2007' and user_name like ('%"&trim(Request("candname"))&"%')"
