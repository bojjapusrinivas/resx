...
if trim(request("stat")) = "A" then
 '' response.write trim(request("stat"))
	...
	'response.write "update TBL_SIS_Analysts set user_status=1 where Rec_id =" & trim(request("recID"))
	con.execute("update TBL_SIS_Applications set user_status=1 where Rec_id =" & trim(request("recID")))
	con.close()
	response.redirect("frm_sis_viewGurgaon.asp")	
end if
...
if trim(request("butnvalue"))<>"" then
	...
	for each var in request.form
		...
		''response.write request("butnvalue") & recID & "<br>"
		''response.end
		con.open "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)  
...
function submitval(actval)
{
	...
	{	
		...
		window.frmsis.action="Frm_SIS_ViewGurgaon.asp?butnvalue="+actval;
...
function OpenWin(num)
	{ 
		myWin = open("ApplicantInfoGurgaon.asp?ID="+num+"#callID1" ,"userwin","width=780,top=5,left=5,height=550,status=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
	}
</script>
...
<table border="1" width="95%" bordercolor="#005563" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#005563"><p align="center"><strong><big><font color="#FFFFFF">Gurgaon Release Requests [<%=ttype%>]</font></big></strong></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<tr><td align="right" colspan="8">[<a href="Frm_SIS_Hold_ViewGurgaon.asp?ttype=8">View Blocked Applications</a>]&nbsp;&nbsp;</td></tr>
	...
	<tr>
		...
		if trim(session("user_type"))="1" then
			if trim(request("candname"))<>"" then		
				usql1="select * from TBL_SIS_Applications where (applicationfor='gurgaon' or applicationfor ='Bizacumen')and location ='gurgaon' and user_name like ('%"&trim(Request("candname"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_Applications where (applicationfor='gurgaon' or applicationfor ='Bizacumen')and location ='gurgaon' and User_Status=0  order by rec_id"
			end if
		else
			if trim(request("candname"))<>"" then
				usql1="select * from TBL_SIS_Applications where (applicationfor='gurgaon' or applicationfor ='Bizacumen')and location ='gurgaon' and user_name like ('%"&trim(Request("candname"))&"%') and user_status=0 order by rec_id"
			else
				usql1="select * from TBL_SIS_Applications where (applicationfor='gurgaon' or applicationfor ='Bizacumen')and location ='gurgaon' and User_Status=0  order by rec_id"
			end if
		end if
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_SIS_ViewGurgaon.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&candname=<%=trim(request("candname"))%>&ttype=<%=ttype1%>"><%=i%></a>			
		...
		IF not rsDisp.EOF THEN
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Response.Write ("<tr bgcolor='white'>")
					...
					Response.Write ("<td align='center'><font face='verdana' size='2'>"& rsDisp("SurName") & " " & rsDisp("User_Name") &"</font></td>")
					...
					if trim(rsDisp("User_Status"))="0" then
						Response.Write ("<td align='center'><a href='javascript:OpenWin("& rsDisp("Rec_id") &");'><font face='verdana' size='2'>View</font></a>  | <a href='FRM_SIS_ViewGurgaon.asp?stat=A&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Release</font></a> | <a href='FRM_SIS_ViewGurgaon.asp?stat=B&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Block</font></a></td>")
					else
