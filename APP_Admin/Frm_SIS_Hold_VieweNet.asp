...
if trim(request("stat")) = "A" then
 '' response.write trim(request("stat"))
	...
	con.close()
	''response.end()
	response.redirect("Frm_SIS_Hold_VieweNet.asp?ttype=8")	
end if
	...
	if trim(var)<>"butnvalue" then
		...
		'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
			...
			cmdEdit.ActiveConnection = con			
				with cmdEdit 				
					.CommandText = "SP_TBL_SIS_FreeLancernew"
			...
			''if loadedval=1 then
				...
				End with
				set mymail = nothing 
				con.execute("update TBL_SIS_FreeLancernew set User_Status=8,Date_Updated=getdate(), Reason_Update=Reason_Update + ' Mail Sent ' where Rec_ID=" & trim(recID))
...
function OpenWin(num)
	{ 
		myWin = open("ApplicantInfo_FreeLancers.asp?ID="+num ,"userwin","width=780,height=550,status=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
	}
function OpenAnaWin(num)
	{ 
		myWin = open("AnAApplicantInfo_FreeLancers.asp?ID="+num+"#callID1" ,"userwin","width=780,top=5,left=5,height=550,status=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
	}
</script>
...
<table border="1" width="95%" bordercolor="#005563" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#005563"><p align="center"><strong><big><font color="#FFFFFF">eNet Blocked Requests</font></big></strong></td>
  </tr>
  ...
  <tr>
	...
	<tr>
		...
		if trim(TStat)="4"  then		
			usql1="select * from TBL_SIS_FreeLancernew  order by rec_id"
		elseif trim(TStat)="0"  then
			usql1="select * from TBL_SIS_FreeLancernew  order by Date_Created desc"
		elseif trim(TStat)="6"  then
			usql1="select * from TBL_SIS_FreeLancernew where ANA_No <> ''  order by Rec_id"
		elseif trim(TStat)="2"  then
			usql1="select * from TBL_SIS_FreeLancernew where user_status="&TStat&"  order by rec_id"
		else
			usql1="select * from TBL_SIS_FreeLancernew where user_status="&TStat&"   order by rec_id"
		end if
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_SIS_Hold_VieweNet.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&ttype=<%=trim(request("ttype"))%>"><%=i%></a>			
