...
If trim(request("ID"))<>"" then
  ...
  If trim(lcase(session("user_name")))<>"superadmin" then
	...
	mysupSQL = "select * from TBL_SIS_Analysts_VSP where Rec_ID="&trim(request("ID"))&""
...
if trim(request("butnvalue"))<>"" then
	...
	elseif trim(lcase(request("butnvalue")))="delete" then
			...
			userrank=0
			userrank1=0
			sqlupdate="update TBL_SIS_Analysts set User_status="& insertval &" and Reason_Update='" & mystat1 & "'  where Rec_ID="&trim(request("ID"))&""
      	end if
		if trim(request("txtreason"))<>"" then
			mystat1 = trim(request("txtreason"))
		else
