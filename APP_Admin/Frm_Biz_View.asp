 ...
 <tr>
  ...
  <tr>
	...
	<tr>
		...
		cpForEdit=cp
	''	con.open "Provider='sqloledb'; Data Source='198.71.225.113'; Initial Catalog='bizacumen_Analyst_vizag'; User ID='newSISuser'; Password='" & Pwd & "';"
	''	con.open "Provider='"&drivertype&"';data source='"&servertype&"';uid='"&userid&"';pwd='"&pwd&"';initial catalog='"&trim(dbname)  & "'"			
		''con.open "Data Source=" & servertype & "; Initial Catalog="&userid&"; User ID="&userid&"; Password='"& pwd& "';"
...
if (trim(session("user_type"))="4" or trim(session("user_type"))="0") then
			if trim(request("candname"))<>"" then		
				usql1="select * from TBL_SIS_Applications where (applicationfor ='Hyderabad' or applicationfor ='Bizacumen') and location ='Hyderabad'  and user_name like ('%"&trim(Request("candname"))&"%') order by rec_id"
			elseif trim(request("candDesc"))<>"" then
				usql1="select * from TBL_SIS_Applications where (applicationfor='Hyderabad'  or applicationfor ='Bizacumen') and location ='Hyderabad'  and Special_Skills like ('%"&trim(Request("candDesc"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_Applications where (applicationfor='Hyderabad' or applicationfor ='Bizacumen') and location ='Hyderabad' and User_Status=0  order by rec_id"
			end if
		else
			if trim(request("candname"))<>"" then
				usql1="select * from TBL_SIS_Applications where (applicationfor='Hyderabad' or applicationfor ='Bizacumen') and location ='Hyderabad'  and user_name like ('%"&trim(Request("candname"))&"%') and user_status=1 order by rec_id"
			elseif trim(request("candDesc"))<>"" then
				usql1="select * from TBL_SIS_Applications where (applicationfor='Hyderabad' or applicationfor ='Bizacumen')and location ='Hyderabad' and Special_Skills like ('%"&trim(Request("candDesc"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_Applications where (applicationfor='Hyderabad' or applicationfor ='Bizacumen')and location ='Hyderabad' and User_Status=1  order by rec_id"
			end if
		end if
