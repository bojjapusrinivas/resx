...
<table border="1" width="95%" bordercolor="#005563" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#3767A6"><p align="center"><strong><big><font color="#FFFFFF">Search eNet Applications [<%=ttype%>]</font></big></strong></td>
  </tr>
  <tr>
...
<td colspan="7" align="center"><font color="#004080"><B>Candidate Email:</B></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <input type="text" name="candEmail" size="20" MAXLENGTH="70"></td></tr>
...
<td colspan="7">  <div align="center"><center><p><input type="submit" value="Enter" name="Btn1"></p>
	...
	<tr>
		...
		if trim(request("ID"))<>"" or trim(request("candPhone")) <> "" or trim(request("candEmail")) <> "" or trim(request("txtSkills"))<>"" then		
			'if trim(session("user_type"))="4"  then
			
				usql1="select * from TBL_SIS_FreeLancernew where  rec_id >0 " &   whrcnd & " order by user_name"
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_eNet_Search.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&candname=<%=trim(request("candname"))%>&txtSurname=<%=trim(request("txtSurname"))%>&candPhone=<%=trim(request("candPhone"))%>&txtSkills=<%=trim(request("txtSkills"))%>&Applicationfor=<%=trim(request("Applicationfor"))%>&id=<%=request("id")%>&ttype=<%=ttype1%>"><%=i%></a>			
