...
if trim(request("stat")) = "A" then
 '' response.write trim(request("stat"))
	...
	'response.write "update TBL_SIS_FreeLancernew set user_status=1 where Rec_id =" & trim(request("recID"))
	con.execute("update TBL_SIS_FreeLancernew set user_status=1 where Rec_id =" & trim(request("recID")))
	con.close()
	response.redirect("frm_biz_vieweNet.asp")	
end if
if trim(request("stat")) = "B" then
 ...
 '' response.write trim(request("stat"))
	con.open "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)  
	set cmdEdit=server.CreateObject("ADODB.Command")
	'response.write "update TBL_SIS_FreeLancernew set user_status=8 where Rec_id =" & trim(request("recID"))
	con.execute("update TBL_SIS_FreeLancernew set user_status=8 where Rec_id =" & trim(request("recID")))
	con.close()
	response.redirect("frm_biz_vieweNet.asp")	
end if
...
if trim(request("butnvalue"))<>"" then
	...
	for each var in request.form
		...
		if trim(lcase(request("butnvalue")))="release" then
			con.execute("update TBL_SIS_FreeLancernew set user_status=1 where Rec_id =" & recID)
			''Response.write ("update TBL_SIS_FreeLancernew set user_status=1 where Rec_id =" & recID)
		end if
		if trim(lcase(request("butnvalue")))="block" then
			con.execute("update TBL_SIS_FreeLancernew set user_status=8 where Rec_id =" & recID)
			''Response.write ("update TBL_SIS_FreeLancernew set user_status=8 where Rec_id =" & recID)
		end if
		if trim(lcase(request("butnvalue")))="graduates release" then
			con.execute("update TBL_SIS_FreeLancernew set user_status=1 , Reason_Update='Graduates Approve' where Rec_id =" & recID)
			''Response.write ("update TBL_SIS_FreeLancernew set user_status=1 and Reason_Update='Graduates Approve' where Rec_id =" & recID)
		end if

		''con.execute("update TBL_SIS_FreeLancernew set user_status=" & insertval & ",Reason_Update='" & mystat1 & "' where Rec_id =" & recID)
...
//	if (updateval==true)
	if (validatefrom()==true)
	{
		...
		if (confirm("Confirm, Do you want this applicant status to "+actval)==true)
		{
			...
			window.frmsis.action="frm_biz_vieweNet.asp?butnvalue="+actval;
...
function OpenWin(num)
	{ 
		myWin = open("ApplicantInfo_FreeLancers.asp?ID="+num+"#callID1" ,"userwin","width=780,top=5,left=5,height=550,status=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
	}
...
<table border="1" width="95%" bordercolor="#005563" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#005563"><p align="center"><strong><big><font color="#FFFFFF">eNet Release Requests [<%=ttype%>]</font></big></strong></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<tr><td align="right" colspan="8">[<a href="Frm_SIS_Hold_VieweNet.asp?ttype=8">View Blocked Applications</a>]&nbsp;&nbsp;</td></tr>
	...
	<tr>
		...
		if (trim(session("user_type"))="4" or trim(session("user_type"))="0") then
			...
			elseif trim(request("candDesc"))<>"" then
				usql1="select * from TBL_SIS_FreeLancernew where  Special_Skills like ('%"&trim(Request("candDesc"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_FreeLancernew where  User_Status=0  order by rec_id"
			end if
		else
			if trim(request("candname"))<>"" then
				usql1="select * from TBL_SIS_FreeLancernew where  user_name like ('%"&trim(Request("candname"))&"%') and user_status=1 order by rec_id"
			elseif trim(request("candDesc"))<>"" then
				usql1="select * from TBL_SIS_FreeLancernew where  Special_Skills like ('%"&trim(Request("candDesc"))&"%') order by rec_id"
			else
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="frm_biz_vieweNet.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&candname=<%=trim(request("candname"))%>&ttype=<%=ttype1%>&candDesc=<%=trim(request("candDesc"))%>"><%=i%></a>			
		...
		IF not rsDisp.EOF THEN
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Dim allapps
				allapps=""
				''response.write "select * from vw_OnlineNaukrieNet_apps where user_phone ='" & trim(rsDisp("user_phone")) & "' or user_email ='" & trim(rsDisp("User_Email")) & "' or user_alternateemail ='" & rsDisp("user_alternateemail") & "'"
				...
				IF not rsAllAps.EOF And  rsAllAps.RecordCount > 1 Then
					 ...
					 while not rsAllAps.EOF 
					   If Trim(rsAllAps("Rec_id")) <> Trim(rsDisp("Rec_id"))   then
						  ...
						  If allapps <> "" then
							allapps= allapps & " ," & "<A href='AnAApplicantIntinfoSearch.asp?ID=" & rsAllAps("Rec_id") & "#callID1' target='new'>" & rsAllAps("Rec_id") & " (O)</a>"
						  Else
						    allapps= "<A href='AnAApplicantIntinfoSearch.asp?ID=" & rsAllAps("Rec_id") & "#callID1' target='new'>" &  rsAllAps("Rec_id") & " (O)</a>"
						  End if
			...
			End if
					...
					if trim(rsDisp("User_Status"))="0" then
						Response.Write ("<td align='center'><a href='javascript:OpenWin("& rsDisp("Rec_id") &");'><font face='verdana' size='2'>View</font></a> | <a href='frm_biz_vieweNet.asp?stat=A&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Release</font></a> | <a href='frm_biz_vieweNet.asp?stat=B&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Block</font></a></td>")
					else
		...
		set rsDisp=nothing
		con.close()%>
		<tr bgcolor='white'><td colspan=8 align=left>&nbsp;&nbsp;<b>[ </b><a style="cursor:hand;" onclick="fnUserCheckAll();">Check All</a> <b>]</b>&nbsp;&nbsp;<b>[</b> <a style="cursor:hand;" onclick="fnUserClearAll();">Clear All</a> <b>]</b></td></tr>
		<%if (session("user_type") = 0) or (session("user_type")=4) then
		Response.Write ("<tr bgcolor='white'>")
			Response.Write ("<td colspan=8 align=left><input type=""Button"" name=""butnvalue"" value=""Release"" onclick=""javascript:submitval('Release')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Block"" onclick=""javascript:submitval('Block');"">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>")
