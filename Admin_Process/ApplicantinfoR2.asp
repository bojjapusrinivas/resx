  ...
  If trim(lcase(session("user_name")))<>"superadmin" then
	...
	If Not rssup.EOF Then
		...
		if trim(ChkRecData)="7" then
			Response.Redirect ("ApplicantInfoR2.asp?ID="&NxtID&"&RecType="&LCRecType&"#callID1")
		end if
...
'response.write "btnval = "&trim(request("butnvalue")) &"<br>"
''Response.end
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
''''''''*********** nEW cODE eND *************''''''''''''''''''''''''''''''''''''
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
<script type='text/javascript'>
	window.opener.location.href="Frm_SIS_Final_ViewR2.asp";
...
if trim(request("butnvalue"))<>"" then 
	...
	elseif trim(lcase(request("butnvalue")))="accept2" then
			...
			'mystat1 = "Approved"
			if trim(request("txtreason"))<>"" then
				mystat1 = trim(request("txtreason"))  ''trim(request("optionsgroup1"))&" "&
				else
			        mystat1 = trim(Mystat) ''trim(request("optionsgroup1"))&" "&
			end if
			userrank1=request("UserRank2")
			
			sqlupdate="update TBL_SIS_Applications set User_Points_US="&trim(userrank1)&" , user_status=2  where Rec_ID="&trim(request("ID"))&""
			sqlInsert = "Insert into Tbl_SIS_ApplicationsStatus values(" & trim(request("ID")) & ",'Accept Stage 2 " & request("txtreason") & "',getdate(),'" &  session("user_name") & "','" & trim(userrank1) & "')"
			insertval = 2
	...
	elseif trim(lcase(request("butnvalue")))="competitor" then
		...
		'	MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>BizAcumen Research Private Limited</td></tr>"
			...
			call SendGodaddyMail("hyderabad@BizAcumen.com",trim(request("txtemail")),"BizAcumen Application Status",MessageBody1)
		''	call SendGodaddyMail("hyderabad@BizAcumen.com","chv@sisinfotech.com","BizAcumen Application Status",MessageBody1)
	...
	con.open "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)  
		...
		if sqlupdate<>"" then
			...
			CON1.open "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)
			con.execute sqlupdate
			if sqlInsert <> "" then
				CON1.execute sqlInsert
			end if
		...
		dim x
		x = cint(Request("ID"))+1
		response.redirect ("ApplicantInfoR2.asp?ID="&trim(x)&"&SUB=1#callID1")
end if
...
function submitval(actval,recID)
{
	...
	//{
		...
		window.frmsis.action="applicantinfoR2.asp?ID="+recID+"&butnvalue="+actval;
...
con.open "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)
if trim(Request("ID"))<>"" then
	'SQL = "SELECT * FROM TBL_SIS_Applications where Rec_id > 474 and Rec_ID="&trim(Request("ID"))
	'SQL = "SELECT * FROM TBL_SIS_Applications where Rec_ID="&trim(Request("ID")) & " and user_status <> 8 "
	SQL = "SELECT * FROM TBL_SIS_Applications where Rec_ID="&trim(Request("ID")) & " and user_status = 2 "
elseif trim(Request("candname"))<>"" then 
	...
	'SQL = "SELECT * FROM TBL_SIS_Applications where Rec_id > 474 and user_name like ('%"&trim(Request("candname"))&"%')"
	SQL = "SELECT * FROM TBL_SIS_Applications where user_name like ('%"&trim(Request("candname"))&"%') and user_status <> 8"
end if
rs.OPEN SQL,con
If Not RS.EOF Then
...
<table border="1" width="90%" cellspacing="0" cellpadding="0" bordercolor="#000080" class=mystyle>
  <tr>
	...
	<%if trim(session("user_type")) ="1" or trim(session("user_type")) ="4" or  trim(session("user_type")) ="5" or  trim(session("user_type")) ="7" then%>
	  ...
	  <tr>
            <td width="17%" height="1" align="left"><font face="verdana" size="2" color="#004080">Father's Name & Occupation</font></td>
            <td width="4%" align="center" height="1"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
            <td width="79%" height="1"><font face="verdana" size="2" color="#004080"><%=rs("Fathers_Name")%>&nbsp;<%=rs("fathers_occupation")%></td>
          </tr>
          <tr>
	...
	<!-- END -->
	 ...
	 <tr align="center">
        <td width="100%" height="0" border=""><form method="post" action="ApplicantinfoR2.asp" name="frmsis">
<tr align="center">
	  ...
	  <tr>
            <td width="100%"><font face="verdana" size="2" color="#004080">Rank2 :</font> <select name="UserRank2">
	...
	<%end if%>
      ...
      <%end if%>
		...
		if trim(session("user_type")) ="4"   then
			...
			Response.Write ("<td colspan=7 align=left><input type=""button"" name=""butnvalue"" value=""Accept Stage II"" onclick=""javascript:submitval('Accept1','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Accept Stage 2"" onclick=""javascript:submitval('Accept2','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Reject"" onclick=""javascript:submitval('Decline','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Hold"" onclick=""javascript:submitval('Hold','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;</td>")	

			''<input type=""button"" name=""butnvalue"" value=""Transfer Gurgaon"" onclick=""javascript:submitval('TGurgaon','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""High Expectations"" onclick=""javascript:submitval('High Expectations','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Competitor"" onclick=""javascript:submitval('Competitor','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Transfer to Graduate"" onclick=""javascript:submitval('TGraduate','"&rs("Rec_ID")&"')"">
			'end if
				'<input type=""button"" name=""butnvalue"" value=""Delete"" onclick=""javascript:submitval('Delete','"&rs("Rec_ID")&"');""
		else
			'if trim(rs("Updated_BY"))<>"superadmin" then
			Response.Write ("<td colspan=7 align=left><input type=""button"" name=""butnvalue"" value=""Accept Stage II"" onclick=""javascript:submitval('Accept2','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Reject"" onclick=""javascript:submitval('Decline','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Hold"" onclick=""javascript:submitval('Hold','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;</td>")	
			''<input type=""button"" name=""butnvalue"" value=""Transfer Gurgaon"" onclick=""javascript:submitval('TGurgaon','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Transfer to Graduate"" onclick=""javascript:submitval('TGraduate','"&rs("Rec_ID")&"')"">
			'end if
		end if					
		Response.Write ("</tr>")
...
Else %>
	...
	<br><p align="left">
		...
		<a href="ApplicantInfoR2.asp?ID=<%=trim(MYVID)%>#callID1"><b> &laquo; Previous </b></a>
