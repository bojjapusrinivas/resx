...
if trim(request("butnvalue"))<>"" then
	...
	'response.end
      	if trim(lcase(request("butnvalue")))="accept1" then
			...
			insertval = 2
			sqlupdate="update TBL_SIS_Applications set User_Points="&trim(userrank)&",reason_update='" & mystat1 & "', user_status=2,Updated_By='" &  trim(session("user_name")) & "' where Rec_ID="&trim(request("ID"))&""
	...
	elseif trim(lcase(request("butnvalue")))="accept3" then
			mystat1=request("txtreason")
			sqlupdate="update TBL_SIS_Applications set admin_status =" & trim(request("SelStatus")) & ", reason_update='" & mystat1 & "'  where Rec_ID="&trim(request("ID"))&""
			insertval = 2
			'admstats=trim(request("SelStatus"))
	...
	elseif trim(lcase(request("butnvalue")))="competitor" then
      	...
      	elseif trim(lcase(request("butnvalue")))="decline" then
			Mystat = "Decline " 
			mystat1 = "Declined "  & request("txtreason")
			userrank=0
			userrank1=0
			insertval = 3
		...
		''	call SendGodaddyMail("hyderabad@BizAcumen.com","aneesa@sisinfotech.com","BizAcumen Application Status",MessageBody1)
			...
			sqlupdate="update TBL_SIS_Applications set user_status =" & insertval & ", reason_update='" & mystat1 & "',Updated_By='" &  trim(session("user_name")) & "'  where Rec_ID="&trim(request("ID"))&""
	elseif trim(lcase(request("butnvalue")))="hold" then
			Mystat = "Hold"
			mystat1 = "Hold " & request("txtreason")
			insertval = 5
			userrank=0
			userrank1=0
...
<table border="0" width="90%" cellspacing="0" cellpadding="0" bordercolor="#000080" class=mystyle>
  <tr>
	...
	IF (trim(rs("user_status"))="8" and trim(lcase(rs("updated_by")))="superadmin") or (trim(rs("user_status"))="9" and trim(lcase(rs("updated_by")))="admin") or (trim(rs("user_status"))="8" and trim(lcase(rs("updated_by")))="admin") or (trim(rs("user_status"))="0" and trim(lcase(rs("updated_by")))="superadmin") then
	%>
...
<%end if%>
      ...
      <tr align="center">
        ...
        <table border="0" width="95%" cellspacing="0" cellpadding="0">
          ...
          if trim(session("user_type")) ="4" Then
			SQL3 = "SELECT * FROM Tbl_SIS_ApplicationsStatus where  Rec_ID="&trim(rs("Rec_ID")) & ""
			'response.write sql1
			rsR2.OPEN SQL3,con
			''response.write sql1
			If Not rsR2.EOF Then%>
				<tr>
				<td width="100%"><font face="verdana" size="2" color="#804040">Stage II <%=UsrStat%>&nbsp;by :&nbsp; <%=rsR2("Updated_BY")%></font></td>
			  </tr>
			   <tr>
				<td width="100%"><font face="verdana" size="2" color="#804040">Stage II Date <%=UsrStat%> :&nbsp; <%=rsR2("Date_Updated")%></font></td>
			  </tr>
			  <tr>
				<td width="100%"><font face="verdana" size="2" color="#804040">Stage II  <%=UsrStat%>&nbsp;Reason:&nbsp; <%=rsR2("Reason_Update")%></font></td>
			  </tr>
			...
			End if %>
	<tr>
            <td width="100%"><font face="verdana" size="2" color="#804040">Candidate Status: <%=adminstat%>&nbsp</font></td>
          </tr>
</table>
        </center></div></td>
      </tr>
	...
	<%if session("user_name")="aneesa" or trim(session("user_type")) ="4" or trim(session("user_type")) ="5" or trim(session("user_type")) ="7" or trim(session("user_type")) ="20" then %>
		  <tr>
	...
	<%if trim(session("user_type")) ="4" Or trim(session("user_type")) ="1" or trim(session("user_type")) ="5"  or trim(session("user_type")) ="7" or trim(session("user_type")) ="20"  then%>
      <tr>
      ...
      <tr>
        ...
        cellspacing="0" cellpadding="0">
		  ...
		  <tr>
            <td width="28%"><font face="verdana" size="2" color="#004080">Currently Salary</font></td>
            <td width="2%" align="center"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
...
<%
if lcase(trim(rs("applicationfor")))="hyderabad" then
SQL1 = "SELECT * FROM Interview_Hyd_Details where  Rec_ID="&trim(rs("Rec_ID")) & ""
else
SQL1 = "SELECT * FROM Interview_Gurgaon_Details where  Rec_ID="&trim(rs("Rec_ID")) & ""
end if
'response.write sql1
rs1.OPEN SQL1,con
''response.write sql1
...
<tr align="center">
		...
		if trim(session("user_type")) = "4"  then
			...
			'end if
		elseif  trim(session("user_type")) = "5" or  trim(session("user_type")) ="7" then
    ...
    <%if trim(session("user_type")) ="4" or trim(session("user_type")) ="5"  or trim(session("user_type")) ="7" then%>
