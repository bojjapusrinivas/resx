...
function OpenWin(num)
	{ 
		myWin = open("ApplicantInfo.asp?ID="+num ,"userwin","width=780,top=5,left=5,height=550,status=no,toolbar=no,menubar=no,scrollbars=yes");
	}
</script>
	...
	<tr>
		...
		if trim(session("user_type"))="1" then
			if trim(request("candname"))<>"" then		
				usql1="select * from TBL_SIS_Analysts where user_name like ('%"&trim(Request("candname"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_Analysts where User_Status=1 and Post_Applied='RESEARCH ANALYST TRAINEE' order by rec_id"
			end if
		else
			if trim(request("candname"))<>"" then
				usql1="select * from TBL_SIS_Analysts where user_name like ('%"&trim(Request("candname"))&"%') and user_status=1 order by rec_id"
			else
				usql1="select * from TBL_SIS_Analysts where User_Status=1 and Post_Applied='RESEARCH ANALYST TRAINEE'  order by rec_id"
			end if
		end if
