...
function OpenWin(num)
	{ 
		myWin = open("ApplicantInfo.asp?ID="+num+"#callID1" ,"userwin","width=780,top=5,left=5,height=550,status=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
	}
</script>
...
<table border="1" width="95%" bordercolor="#3767A6" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#3767A6"><p align="center"><strong><big><font color="#FFFFFF">BizAcumen Pending Requests [<%=ttype%>]</font></big></strong></td>
  </tr>
  <tr>
	...
	<tr>
		...
		if (trim(session("user_type"))="1" or trim(session("user_type"))="4") then
			if trim(request("candname"))<>"" then		
				usql1="select * from TBL_SIS_Applications where  user_name like ('%"&trim(Request("candname"))&"%') order by rec_id"
			elseif trim(request("candPhone"))<>"" then 
				usql1="select * from TBL_SIS_Applications where  user_phone like ('%"&trim(Request("candPhone"))&"%') order by rec_id"
			else
				if (trim(session("user_type"))="1") then
					usql1="select * from TBL_SIS_Applications where  User_Status=1 and admin_status =1 order by rec_id"  '''AND (Reason_Update IS NULL) order by rec_id     ''and reason_update Not like ('%Graduates%') 
				else
					usql1="select * from TBL_SIS_Applications where  User_Status=1 and admin_status =1  order by rec_id"
				end if
			end if
		else
			if trim(request("candname"))<>"" then
				usql1="select * from TBL_SIS_Applications where  user_name like ('%"&trim(Request("candname"))&"%') and user_status=1 order by rec_id"
			elseif trim(request("candPhone"))<>"" then 
				usql1="select * from TBL_SIS_Applications where  user_phone like ('%"&trim(Request("candPhone"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_Applications where  User_Status=1 and admin_status =1  order by rec_id"
	...
	'	response.write usql1
		...
		if i>cint(rec_per_page) then
			...
			for i=1 to rsDisp.PageCount%>			
				<a href="Frm_SIS_ViewAll.asp?Start=<%=Start%>&amp;E=<%=E%>&amp;Cp=<%=i%>&candname=<%=trim(request("candname"))%>&ttype=<%=ttype1%>"><%=i%></a>			
		...
		IF not rsDisp.EOF THEN
			...
			'			  If allapps <> "" then
			'				allapps= allapps & ", " & "<a href='AnAApplicantIntinfoSearchNu.asp?ID=" & rsAllAps("Rec_id") & "#callID1' target='new'>" &  rsAllAps("Rec_id") & " (N)</a>"
			'			  Else
			'			    allapps=  "<a href='AnAApplicantIntinfoSearchNu.asp?ID=" & rsAllAps("Rec_id") & "#callID1' target='new'>" & rsAllAps("Rec_id") & " (N)</a>"
			'			  End if
			'			Else
			'			 If allapps <> "" then
			...
			'	set rsAllAps=nothing
				...
				Response.Write ("<tr bgcolor='white'>")
					...
					Response.Write ("<td align='center'><font face='verdana' size='2'>"& rsDisp("User_Name") & " " &  rsDisp("surName") & "</font></td>")
