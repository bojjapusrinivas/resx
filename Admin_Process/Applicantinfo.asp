...
'response.end	
  If trim(lcase(session("user_name")))<>"superadmin" then
	...
	mysupSQL = "select * from TBL_SIS_Applications where Rec_ID="&trim(request("ID"))&" and user_status=1"
	...
	If Not rssup.EOF Then
		...
		if trim(ChkRecData)="7" then
			Response.Redirect ("ApplicantInfo.asp?ID="&NxtID&"&RecType="&LCRecType&"#callID1")
		end if
...
if trim(lcase(request("butnvalue")))="tgurgaon" then
	...
	sqltrfupdate="update TBL_SIS_Applications set Applicationfor='Gurgaon' where rec_id =" & trim(request("ID"))
...
if trim(request("butnvalue"))<>"" then 
		...
		dim Mystat, mystat1, insertval,userrank,userrank1,MessageBody1
		userrank="0"
		userrank1="0"
      	...
      	if trim(lcase(request("butnvalue")))="accept1" then
			...
			insertval = 2
			sqlupdate="update TBL_SIS_Applications set Date_Updated=getdate() ,Reason_Update='" & mystat1 & "',Updated_By='" & session("user_name") & "', user_status=2,Applicant_Status=2 where Rec_ID="&trim(request("ID"))&""
			sqlInsert = "Insert into Tbl_SIS_ApplicationsR1Status values(" & trim(request("ID")) & ",'Accept Stage 1 " & request("txtreason") & "',getdate(),'" &  session("user_name") & "','" & trim(userrank) & "')"
	elseif trim(lcase(request("butnvalue")))="tgraduate" then
			'Mystat = "Approve"
			'mystat1 = "Approved"
			if trim(request("txtreason"))<>"" then
				mystat1 = trim(request("optionsgroup1"))&" "&trim(request("txtreason"))
			 ...
			 mystat1 = "Graduates Approved"
			userrank=request("UserRank")
			insertval = 2
			sqlupdate="update TBL_SIS_Applications set Date_Updated=getdate() ,Reason_Update='" & mystat1 & "',Updated_By='" & session("user_name") & "', user_Status=2 where Rec_ID="&trim(request("ID"))&""
	...
	elseif trim(lcase(request("butnvalue")))="accept2" then
			...
			sqlupdate="update TBL_SIS_Applications set User_Points_US="&trim(userrank1)&" , user_status=2,Applicant_Status=2 where Rec_ID="&trim(request("ID"))&""
			insertval = 2
	...
	elseif trim(lcase(request("butnvalue")))="high expectations" then
			...
			sqlupdate="update TBL_SIS_Applications set user_status =9,Reason_Update='" & mystat1 & "',Applicant_Status=9,Applicant_Remarks='" & mystat1 & "'  where Rec_ID="&trim(request("ID"))&""
			insertval = 9
	...
	elseif trim(lcase(request("butnvalue")))="competitor" then
			...
			sqlupdate="update TBL_SIS_Applications set user_status =10,Reason_Update='" & mystat1 & "',Applicant_Status=10,Applicant_Remarks='" & mystat1 & "'  where Rec_ID="&trim(request("ID"))&""
			insertval = 10
			'admstats=trim(request("SelStatus"))
			rESPONSE.WRITE SQLUPDATE
      	elseif trim(lcase(request("butnvalue")))="decline" then
			...
			MessageBody1 = MessageBody1 & "<tr><td ><font face='arial' size='2'>Thank you for taking out time and applying for a job position at our company. We regret to inform you that our Recruitment and Selection Department has not approved your online application based on several parameters. Subsequently, we wish you good luck in all your future professional endeavors. We once again thank you for your interest in working for us.<br><br></td></tr>"
		...
		''	call SendGodaddyMail("careers@BizAcumen.com",trim(request("txtemail")),"BizAcumen Application Status",MessageBody1)
		''	call SendGodaddyMail("careers@BizAcumen.com","chv@BizAcumen.com","BizAcumen Application Status",MessageBody1)
		''	call SendGodaddyMail("hyderabad@BizAcumen.com","aneesa@sisinfotech.com","BizAcumen Application Status",MessageBody1)

			sqlupdate="update TBL_SIS_Applications set user_status =3,Date_Updated=getdate() ,Reason_Update='" & mystat1 & "',Applicant_Status=3,Applicant_Remarks='" & mystat1 & "'  where Rec_ID="&trim(request("ID"))&""
	elseif trim(lcase(request("butnvalue")))="hold" then
			...
			userrank=0
			userrank1=0
			sqlupdate="update TBL_SIS_Applications set user_status =5,Reason_Update='" & mystat1 & "',Applicant_Status=5,Applicant_Remarks='" & mystat1 & "'  where Rec_ID="&trim(request("ID"))&""
	elseif trim(lcase(request("butnvalue")))="delete" then
			...
			userrank=0
			userrank1=0
			sqlupdate="update TBL_SIS_Applications set User_status="& insertval &" ,Date_Updated=getdate() , Reason_Update='" & mystat1 & "',Updated_By='" & session("user_name") & "',Applicant_Status=" & insertval & ",Applicant_Remarks='" & mystat1 & "'  where Rec_ID="&trim(request("ID"))&""
      	end if
		if trim(request("txtreason"))<>"" then
			mystat1 = trim(request("optionsgroup1"))&" "&trim(request("txtreason"))
		else
	...
	con.open "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)  
		if trim(sqlupdate)="" then
			...
			cmdEdit.ActiveConnection = con	
			with cmdEdit
				.CommandText = "SPTBL_SIS_Applications"
				.CommandType = adcmdStoredProc	
				.Parameters.Append .CreateParameter ("@MyOption",advarchar,adparamInput,10,"UPDATESTAT")
...
con.open "Provider="&drivertype&";data source="&servertype&";uid="&userid&";pwd="&pwd&";initial catalog="&trim(dbname)
if trim(Request("ID"))<>"" then
	'SQL = "SELECT * FROM TBL_SIS_Applications where Rec_id > 0 and Rec_ID="&trim(Request("ID"))
	'SQL = "SELECT * FROM TBL_SIS_Applications where Rec_ID="&trim(Request("ID")) & " and user_status <> 8 "
	SQL = "SELECT * FROM TBL_SIS_Applications where Rec_ID="&trim(Request("ID")) & " and user_status = 1 "
elseif trim(Request("candname"))<>"" then 
	SQL = "SELECT * FROM TBL_SIS_Applications where user_name like ('%"&trim(Request("candname"))&"%') and user_status <> 8"
else
	'SQL = "SELECT * FROM TBL_SIS_Applications where Rec_id > 0 and user_name like ('%"&trim(Request("candname"))&"%')"
	SQL = "SELECT * FROM TBL_SIS_Applications where user_name like ('%"&trim(Request("candname"))&"%') and user_status <> 8"
end if
'response.write sql
'response.end
  ...
  <tr>
    ...
    cellspacing="0" cellpadding="5">
	  ...
	  <tr>
            ...
            <td width="79%" height="1"><font face="verdana" size="2" color="#008080"><%=rs("Location")%></td>
          </tr>
          <tr>
            <td width="17%" height="1" align="left"><font face="verdana" size="2" color="#004080">Name & Surname</font></td>
            <td width="4%" align="center" height="1"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
            <td width="79%" height="1"><font face="verdana" size="2" color="#008080"><%=rs("user_Name")%> &nbsp;<%=rs("surName")%></td>
          </tr>
		 <tr>
		  ...
		  <tr>
            <td width="17%" height="25" align="left"><font face="verdana" size="2" color="#004080">Disabilities if any</font></td>
            <td width="4%" align="center" height="25"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
            <td width="79%" height="25"><font face="verdana" size="2" color="#008080"><%=rs("Access_Skills")%></td>
          </tr>
	...
	<%if trim(session("user_type")) ="1" or trim(session("user_type")) ="4" or  trim(session("user_type")) ="5" or  trim(session("user_type")) ="7" then%>
	      ...
	      <tr>
            ...
            <td width="4%" align="center" height="1"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
            <td width="79%" height="1"><font face="verdana" size="2" color="#008080"><%=Replace(Replace(rs("Fathers_Name"),"##", "  "),"Mother`s Name:","<font face='verdana' size='2' color='#004080'><b>&nbsp; & &nbsp;</b> Mother's Name:</font>")%></td>
          </tr>
		  <tr>
          ...
          <tr>
            <td width="17%" align="left"><font face="verdana" size="2" color="#004080">Present Address</font></td>
	...
	<%end if%>
 	...
 	<tr>
            <td width="28%" height="25"><font face="verdana" size="2" color="#004080">Percentage Scored </font></td>
            <td width="2%" height="25" align="center"><font face="verdana" size="2" color="#004080"><strong>:</strong></font></td>
            <td width="77%" height="25"><font face="verdana" size="2" color="#008080"><%=rs("Percentage_scored")%></td>
          </tr>
        </table>
        </center></div></td>
      </tr>
      ...
      <tr>
        <td width="100%" height="10" bgcolor="#9DC3F5"><big><font face="verdana" size="2" color="#804040"><strong>&nbsp;&nbsp; Employment</strong></font></big></td>
      </tr>
      <tr>
      ...
      <tr align="center">
        <td width="100%" height="6"><div align="center"><center><table border="0" width="95%" cellspacing="0">
            ...
            <td  align="left"><font face="verdana" size="2" color="#008080"><b>Tasks Performed:</b><br><%=rs("Tasks_performed")%></td>
          </tr>
        </table>
        </center></div></td>
      </tr>
      ...
      <tr align="center">
        <a name="callID1"><td width="100%" height="1" bgcolor="#9DC3F5"><p align="left"><big><font face="verdana" size="2" color="#804040"><strong>&nbsp;&nbsp;
      ...
      <tr align="center">
        ...
        <table border="1" width="98%" bordercolor="#800000" cellspacing="0" cellpadding="3">
          <tr>
            <td width="100%"><font face="verdana" size="2" color="#3767A6"><%=rs("Special_Skills")%></td>
          </tr>
        </table>
        </center></div></td>
      </tr>
...
<tr align="center">
		...
		<table width="98%" cellspacing="0" cellpadding="3">
         ...
         <!-- <tr>
            <td width="100%"><font face="verdana" size="2" color="#004080">Give Reason if any :</font> <textarea id="txtreason" visible="false" name="txtreason"></textarea></td>
          </tr> -->
	   <tr>
            <td width="100%"><font face="verdana" size="2" color="#004080">
		<INPUT Type="radio" id="optionsgroup1" name="optionsgroup1" value="BDG" Selected >BDG&nbsp;&nbsp;
	...
	<!--	<INPUT Type="radio" id="optionsgroup1" name="optionsgroup1" value="RSG-I" >RSG-I&nbsp;&nbsp;-->
		<INPUT Type="radio" id="optionsgroup1" name="optionsgroup1" value="SERT" >SERT&nbsp;&nbsp;
		...
		<INPUT Type="radio" id="optionsgroup1" name="optionsgroup1" value="Virtual Assistant" >Virtual Assistant&nbsp;&nbsp;&nbsp;
		</td> 
          </tr>
...
<!--	  <tr>
	...
	<input type="hidden" name="UserRank" value="0">
	...
	<%end if%>
      ...
      <%end if%>
		...
		if trim(session("user_type")) ="4"   then
			...
			''Response.Write ("<td colspan=7 align=left><input type=""button"" name=""butnvalue"" value=""Accept Stage 1"" onclick=""javascript:submitval('Accept1','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Accept Stage 2"" onclick=""javascript:submitval('Accept2','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Reject"" onclick=""javascript:submitval('Decline','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Hold"" onclick=""javascript:submitval('Hold','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Transfer Gurgaon"" onclick=""javascript:submitval('TGurgaon','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""High Expectations"" onclick=""javascript:submitval('High Expectations','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Competitor"" onclick=""javascript:submitval('Competitor','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<!--<input type=""button"" name=""butnvalue"" value=""Transfer to Graduate"" onclick=""javascript:submitval('TGraduate','"&rs("Rec_ID")&"')"">--></td>")	
			Response.Write ("<td colspan=7 align=left><input type=""button"" name=""butnvalue"" value=""Accept"" onclick=""javascript:submitval('Accept1','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Accept Stage 2"" onclick=""javascript:submitval('Accept2','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Reject"" onclick=""javascript:submitval('Decline','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Hold"" onclick=""javascript:submitval('Hold','"&rs("Rec_ID")&"');""><input type=""button"" name=""butnvalue"" value=""High Expectations"" onclick=""javascript:submitval('High Expectations','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Competitor"" onclick=""javascript:submitval('Competitor','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<!--<input type=""button"" name=""butnvalue"" value=""Transfer to Graduate"" onclick=""javascript:submitval('TGraduate','"&rs("Rec_ID")&"')"">--></td>")	
			'end if
				'<input type=""button"" name=""butnvalue"" value=""Delete"" onclick=""javascript:submitval('Delete','"&rs("Rec_ID")&"');""
		else
			'if trim(rs("Updated_BY"))<>"superadmin" then
		''	Response.Write ("<td colspan=7 align=left><input type=""button"" name=""butnvalue"" value=""Accept Stage 1"" onclick=""javascript:submitval('Accept1','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Reject"" onclick=""javascript:submitval('Decline','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Hold"" onclick=""javascript:submitval('Hold','"&rs("Rec_ID")&"');"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Transfer Gurgaon"" onclick=""javascript:submitval('TGurgaon','"&rs("Rec_ID")&"')"">&nbsp;&nbsp;&nbsp;<input type=""button"" name=""butnvalue"" value=""Transfer to Graduate"" onclick=""javascript:submitval('TGraduate','"&rs("Rec_ID")&"')""></td>")	
