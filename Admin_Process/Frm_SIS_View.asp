...
if trim(request("butnvalue"))<>"" then
	...
	for each var in request.form
		...
		con.execute("update TBL_SIS_Applications set user_status=" & insertval & ",Reason_Update='" & mystat1 & "',Applicant_Status=" & insertval & ",Applicant_Remarks='" & mystat1 & "' where Rec_id =" & recID)
...
<table border="1" width="95%" bordercolor="#3767A6" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"> 
 <tr>
    <td width="100%" bgcolor="#3767A6"><p align="center"><strong><big><font color="#FFFFFF">Hyderabad Pending Requests [<%=ttype%>]</font></big></strong></td>
  </tr>
  <tr>
	...
	<tr>
		...
		if (trim(session("user_type"))="1" or trim(session("user_type"))="4" or trim(session("user_type"))="5" or trim(session("user_type"))="7") then
			if trim(request("candname"))<>"" then		
				usql1="select * from TBL_SIS_Applications where Applicationfor='Bizacumen' and user_name like ('%"&trim(Request("candname"))&"%') order by rec_id"
			elseif trim(request("candPhone"))<>"" then 
				usql1="select * from TBL_SIS_Applications where Applicationfor='Bizacumen' and user_phone like ('%"&trim(Request("candPhone"))&"%') order by rec_id"
			else
				if (trim(session("user_type"))="5" or trim(session("user_type"))="7") then
					usql1="select * from TBL_SIS_Applications where Applicationfor='Bizacumen' and User_Status=1 and reason_update like ('%Graduates%')  order by rec_id"
				else
					usql1="select * from TBL_SIS_Applications where Applicationfor='Bizacumen' and User_Status=1  order by rec_id"
				end if
			end if
		else
			if trim(request("candname"))<>"" then
				usql1="select * from TBL_SIS_Applications where Applicationfor='Bizacumen' and user_name like ('%"&trim(Request("candname"))&"%') and user_status=1 order by rec_id"
			elseif trim(request("candPhone"))<>"" then 
				usql1="select * from TBL_SIS_Applications where Applicationfor='Bizacumen' and user_phone like ('%"&trim(Request("candPhone"))&"%') order by rec_id"
			else
				usql1="select * from TBL_SIS_Applications where Applicationfor='Bizacumen' User_Status=1  order by rec_id"
			end if
		end if
		...
		IF not rsDisp.EOF THEN
			...
			while not rsDisp.EOF and rowcount<rsDisp.PageSize
				...
				Response.Write ("<tr bgcolor='white'>")
					...
					if trim(rsDisp("User_Status"))="1" then
						Response.Write ("<td align='center'><a href='javascript:OpenWin("& rsDisp("Rec_id") &");'><font face='verdana' size='2'>View</font></a></td>") ' | <a href='FRM_SIS_view.asp?stat=A&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Accept</font></a> | <a href='FRM_SIS_view.asp?stat=D&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Decline</font></a> | <a href='FRM_SIS_view.asp?stat=H&RecID="&rsDisp("Rec_id")&"&emailid="&rsDisp("User_Email")&"'><font face='verdana' size='2'>Hold</font></a></td>")
					else
